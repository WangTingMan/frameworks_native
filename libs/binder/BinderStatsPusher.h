...
#ifdef _MSC_VER
template <typename T>
size_t hash_combine( size_t h, const T& val ) {
    std::hash<T> hasher;
    h ^= hasher( val ) + 0x9e3779b9 + (h << 6) + (h >> 2);
    return h;
}
#endif
...
#ifdef _MSC_VER 
            h = hash_combine( h, std::hash<std::u16string_view>{}(bcd.interfaceDescriptor) );
            h = hash_combine( h, std::hash<uint32_t>{}(bcd.transactionCode) );
#else
